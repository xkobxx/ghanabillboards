...
import { LayoutDashboard, Presentation, Route, LineChart, BadgeDollarSign, Settings, ShieldAlert, MailCheck } from 'lucide-react';
...
const DECK_SLIDES = [
  { num: '01', title: 'Problem', body: 'African OOH advertising runs on email chains, static PDFs, and phone calls. Advertisers can\'t discover, compare, or book inventory without manual broker involvement.' },
  { num: '02', title: 'Solution', body: 'Vantage Point is a unified terminal that gives advertisers one public entry point, then routes booking, vendor, operator, and investor workflows into role-specific spaces.' },
  ...
  { num: '04', title: 'Business model', body: 'Marketplace commission on bookings, vendor SaaS subscription, premium analytics products, and future programmatic exchange infrastructure fee.' },
  ...
  { num: '06', title: 'Architecture', body: 'Modular monolith today, transitioning to microservices as booking volume and vendor node count crosses the 500-node threshold.' },
];
    ...
    >
            ...
            <section className={`vp-dashboard-view${activeView === 'overview' ? ' active' : ''}`}>
              ...
              <div className="vp-panel-col-grid three">
                {[
                  ...
                  { title: 'Marketplace model', body: 'Separates booking commission, vendor SaaS, analytics, and future exchange infrastructure.' },
                ].map(({ title, body }) => (
            ...
            <section className={`vp-dashboard-view${activeView === 'roadmap' ? ' active' : ''}`}>
              <article className="vp-dash-panel">
                ...
                <div className="vp-dash-list">
                  {[
                    { phase: 'Phase 1', label: 'Unified booking terminal', status: 'Live' },
                    { phase: 'Phase 2', label: 'Live IoT telemetry integration', status: 'Q4 2026' },
                    { phase: 'Phase 3', label: 'Programmatic RTB exchange', status: '2027' },
                  ].map(({ phase, label, status }) => (
                    <div key={phase} className="vp-dash-item">
                      <span>{label}</span>
            ...
            <section className={`vp-dashboard-view${activeView === 'model' ? ' active' : ''}`}>
              <article className="vp-dash-panel">
                ...
                <div className="vp-dash-list">
                  {[
                    { label: 'Marketplace commission', value: 'Booking layer' },
                    { label: 'SaaS subscription', value: 'Vendor / operator tools' },
                    { label: 'Premium analytics', value: 'Telemetry data products' },
                  ].map(({ label, value }) => (
                    <div key={label} className="vp-dash-item">
                    ...
                    {editingProfile ? (
                      <>
                        ...
                        <div className="vp-dash-item" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 6 }}>
                          <span>Bio</span>
                          <textarea value={profileDraft.bio} onChange={e => setProfileDraft(d => ({ ...d, bio: e.target.value }))} className="vp-input-inline" rows={3} style={{ resize: 'vertical' }} placeholder="Tell us about your firm..." />
                        </div>
                      </>
                    ) : (
                      <>
                ...
                <article className="vp-dash-panel">
                  ...
                  <div className="vp-dash-list">
                    {editingSettings ? (
                      ...
                      <>
                        <div className="vp-dash-item"><span>Deck access level</span><strong>{iSettings.deckAccess === 'teaser' ? 'Teaser only' : iSettings.deckAccess === 'full' ? 'Full deck' : 'Architecture material'}</strong></div>
                        <div className="vp-dash-item"><span>Update cadence</span><strong>{iSettings.updateCadence.charAt(0).toUpperCase() + iSettings.updateCadence.slice(1)}</strong></div>
                        <div className="vp-dash-item"><span>NDA requirement</span><strong>{iSettings.ndaRequired ? 'Required' : 'Not required'}</strong></div>
                        <div className="vp-dash-item"><span>Email alerts</span><strong>{iSettings.emailAlerts ? 'Enabled' : 'Disabled'}</strong></div>
                        <div className="vp-dash-item"><span>SMS alerts</span><strong>{iSettings.smsAlerts ? 'Enabled' : 'Disabled'}</strong></div>
                      </>
                    )}
                  </div>
                </article>
